...
**Assumptions:** 
- Variables $X,Y$ are identically and independently distributed with identities $N(\mu_X,\sigma^2)$ and $N(\mu_Y,\sigma^2)$.
...
sd = sd(df$diff)
dbar = mean(df$diff)
t0 = t.test(df$diff,mu=0)$statistic
pval = t.test(df$diff,mu=0)$p.value
df = n-1
```
