...
mu = 30
S = sd(data$uni_work)
(test = t.test(data$uni_work,mu = 130,alternative = "two.sided"))
...
- The hypotheses can bbe stated interms of *mean* or *median* since we assume that the distribution is symmetric
...
# Wilcoxon rand-sum test - for two samples

**Wilcoxon rand-sum test / Mann-Whitney U test:**
...
- **Disadvantage:** if one is primarily interested in differences in location between the two distributions, the Wilcoxon test has the disadvantage of also reacting to other differences between the distriutions such as differences in shape.
...
**Rule of thumb:** if the notches of the two boxes do not overlap, this sugests that hte medians are significantly different.
