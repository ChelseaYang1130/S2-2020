...
```
As shown in Figure \@ref(fig:covidplot), the peak of the number of times students have been tested for COVID lies to the left with a long tail extending to the right. Therefore the data is significantly right-skewed.
...
### Is stress level related to gender?

```{r}
x %>% ggplot() + 
  aes(y = stress_level, fill = gender) + 
  geom_bar(position="fill") + 
  labs(y = "Stress level", x = "Proportion of students", fill="Gender",
       title = "DATA2002 students stress level") +
  scale_x_continuous(labels = scales::percent_format())+theme(plot.title = element_text(hjust = 0.5))
```

The size of contingency table is $3 \times 11$. In this case, Fisher's exact test can be generalized to $3 \times 11$ two-way contingency tables but is very dificult to compute. Therefore we use Monte Carlo (i.e. random permutation) for the hypothesis testing.

*Frequency table:*
```{r}
(y.mat = table(x$stress_level,x$gender))
```

**Hypothsis:** $H_0:$ gender and stress level are independent ($p_{ij} =p_ip_j, i=1,2, j=1,2,3,4$) **vs** $H_1:$ gender and stress level are related (Not all equalities hold).

**Assumptions:** No assumptions are made

**Test statistics:** $t_0=19.258$
```{r}
set.seed(100)
...
**Conclusion:** The p-value is greater than 0.05. The data is consistent with $H_0$ therefore we conclude that gender and stress level are independent.
...
x1 %>% ggplot() + 
  ...
  labs(y = "", x = "Count", title = "DATA2002 students' favorite social media")+theme(plot.title = element_text(hjust = 0.5))
```
...
https://www.codecademy.com/learn/learn-statistics-with-python/modules/describe-a-histogram#:~:text=In%20a%20histogram%2C%20the%20distribution,a%20symmetric%20dataset%20are%20similar.

https://socratic.org/questions/what-is-the-difference-between-a-chi-square-test-of-independence-and-a-chi-squar#:~:text=Test%20for%20independence%20is%20concerned,samples%20come%20from%20same%20population.
