...
- The hypotheses can be stated interns of *mean* or *median* since we assume that the distribution is symmetric
...
**Test statistic:** $W=R_1+R_2+...+R_{n_X}$. Under $H_0$, the test statistic follows the $WRS(`r nA`,`r nB`)$ distribution.
...
```{r fig.cap="Distribution of test statistics."}
...
- for a 95% CI, 95% of all possible random samples will contain the population mean, leading to 95% **confidence** that a single interval contains the true mean.

for a 95% CI (a,b):

- if we draw a large number of random samples and compute for each sample a 95% CI, about 95% of these CIs will contain $\mu$.
- CI is a range of plausible values for the population parameter.
...
- Provides answers to problems for which analytic solutions are impossble.
...
```{r fig.cap="Histogram of boostraping results"}
data.frame(result) %>%
ggplot(aes(x = result)) +
...
ggplot(data, aes(x= gender,y=exercise,fill=gender))+
  geom_boxplot(aes(x = gender, y = exercise), outlier.alpha = 0.5)+labs(y = "Count", x = "Exercise")
```
...
data %>% ggplot() + 
  aes(x = social_media,fill = gender, stat = "count")   +
  geom_bar() + 
...
data %>% ggplot() + 
  ...
  scale_x_discrete(name=" ", limits=as.character(sort$Var1)) +
  labs(x = "", y = "Count")+theme(plot.title = element_text(hjust = 0.5))+
  coord_flip()
...
```{r}
ggplot(data, aes(sample=exercise,color=gender)) + geom_qq() + geom_qq_line()+facet_wrap(~ gender)+theme_classic()+theme_linedraw() + theme(legend.position = "none")
```
