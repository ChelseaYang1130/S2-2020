...
hand, the number of win shares which is linearly related to salary is
also found to be correlated with number of offensive win shares and
number of defensive win shares. One the other hand, total rebound is
linearly related to both offensive rebound and defensive rebound.
...
\hline
\textbf{KNN regression} & 4.1927 & 4.0804 \\ 
...
three models selected. As shown in Table \ref{table:errors}, predictive
performances of three models are satisfactory as all test errors are
less the 4.1 millions dollars. The lasso regression model with
\(\alpha=0.5\) has the lowest validation and testing error, at 3.5857
and 3.957 (\$ Millions) respectively. Thus the lasso regression model is
the best-suited the data with greatest predictive performance. The root
mean squared error of test set indicates that the standard deviation of
the unexplained variance of salary is 3.957 millions dollars. Moreover,
it is also reflected from Table \ref{table:errors} that a model with a
lower validation error tends to have a lower testing error.
